...
sidc <- sidc %>% filter(Ymd <="2019-12-31") %>% select(Ymd,R)
# df_s <- sidc %>% filter(Ymd >="2009-01-01" & Ymd <="2019-12-31")
```

# Combined Kanzel & Sidc data sets and calc Spots and Groups
```{r Use sidc to fill in missing kanzel dates}
...
# scottcame @ r4ds community
# kc %>% mutate(g=case_when(is.na(g) ~ R %/% 10, TRUE ~ g))
# kc %>% mutate(s=case_when(is.na(s) ~ R %/% 10, TRUE ~ s))
#
...
```
```{r Comparing actual to predicted isn for Jan. 2020}
fcast1 <- forecast %>% filter(ds >="2020-01-01")
isn1 <- kd %>% filter(Ymd >="2010-01-01")
ggplot(fcast1) + geom_line(x=ds,y=yaht,col="yhat") +
